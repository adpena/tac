...
SCRIPT = REPO / "scripts" / "remote_lane_i_darts_dims.sh"
...
def test_script_exists():
    assert SCRIPT.exists(), f"missing Lane I-DARTS script: {SCRIPT}"
...
def test_completion_marker_present(script_text: str):
    assert "LANE_I_DARTS_DONE" in script_text
...
def test_log_dir_namespaced(script_text: str):
    assert "lane_i_darts_results" in script_text


def test_tag_namespaced(script_text: str):
    assert 'TAG="lane_i_darts_dims"' in script_text


def test_internal_lane_name_lane_i_darts(script_text: str):
    assert "[lane-i-darts]" in script_text
    assert "lane=I-DARTS" in script_text
...
def test_candidate_hidden_dims_recorded(script_text: str):
    """Pre-flight + provenance both must reference the canonical lists."""
    assert "(8, 16, 24, 32)" in script_text


def test_imports_coolchic_supernet(script_text: str):
    assert "build_coolchic_supernet" in script_text
